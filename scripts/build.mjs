...
/**
 * 读取根目录 `package.json` 并立即写回（仅做格式化规范化，不改变语义）。
 * 这样单次构建即可统一 `package.json` 与其它清单的排版风格。
 *
...
const getPkg = () => {
  ...
  // 写回同路径：依赖 JSON.stringify 的稳定键序与缩进，避免仅「读」时与磁盘内容仅有空白差异
  saveJson(fn, json);
...
/**
 * 更新 `.claude-plugin/marketplace.json`：顶层 `name` 固定为 `{pkg.name}-dev`，描述带「开发市场」前缀；
